...
class VictoryController: InterfaceController{
    @IBOutlet weak var lblSubtitles: WKInterfaceLabel!
      var player:AVQueuePlayer?
      var phrasesArray = [  "Hercules: I did it!"]
    

            
            override func awake(withContext context: Any?) {
                  super.awake(withContext: context);
                  MusicHelper.sharedHelper.playBackgroundMusic(Name: "VictoryMusic", Path: "/Audio/Themes")

                    let soundsArray = GetFilesFromFolder(path: "/Audio/HydraDying")
                  ...
                  let sortedArray = soundsArray.sorted { $0.lastPathComponent < $1.lastPathComponent }
                        ...
                        let path = Bundle.main.path(forResource: audioString, ofType: "mp3", inDirectory: "/Audio/HydraDying")!
                   ...
                   // Slow the dialoug down and speed it up appropriately
                           ...
                           if (phrasesArray.count>0){phrasesArray.removeFirst() }
                               ...
                               DispatchQueue.main.asyncAfter(deadline: .now() + 0.70, execute: {
                                self.player?.pause()
                                    self.pushController(withName: "SuccesScene", context: nil)
                                })
